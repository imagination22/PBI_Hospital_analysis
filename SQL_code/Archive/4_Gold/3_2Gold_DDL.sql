...
ADD 
    department_skey INT FOREIGN KEY REFERENCES GOLD.Dim_Department(department_skey),
    status VARCHAR(50),
    joining_date DATE,
    ...
    email VARCHAR(255),
    image_path VARCHAR(500);
ALTER TABLE GOLD.Fact_Appointments
ADD 
    time_skey INT FOREIGN KEY REFERENCES GOLD.Dim_Time(time_skey),
    status VARCHAR(50),
    doctor_notes VARCHAR(MAX),
    suggestions VARCHAR(MAX);
	ALTER TABLE GOLD.Fact_Tests
...
ADD 
	...
	ALTER TABLE GOLD.Dim_Medicine
ADD 
    manufacturing_date DATE,
    batch_number VARCHAR(50),
    reorder_level INT;
	ALTER TABLE GOLD.Fact_MedicinePurchases
ADD 
...
CREATE TABLE GOLD.Agg_MonthlyRevenue (
    ...
    num_patients INT,
    num_bills INT,
    PRIMARY KEY (month_skey, department_skey),
    FOREIGN KEY (department_skey) REFERENCES GOLD.Dim_Department(department_skey)
);
...
CREATE TABLE GOLD.Agg_DoctorPerformance (
    ...
    total_fees DECIMAL(15, 2),
    avg_fees_per_appointment AS (CASE WHEN total_appointments = 0 THEN 0 ELSE total_fees / total_appointments END) PERSISTED,
    num_feedbacks INT,
    PRIMARY KEY (month_skey, doctor_skey),
    FOREIGN KEY (doctor_skey) REFERENCES GOLD.Dim_Doctor(doctor_skey)
);
...
CREATE TABLE GOLD.Agg_BedOccupancy (
  ...
  --  department_skey INT,
    ...
    num_admissions INT,
    turnover_rate DECIMAL(5, 2), -- admissions / total_beds
...
CREATE TABLE GOLD.Agg_TestAnalytics (
    ...
    PRIMARY KEY (test_skey, month_skey)
);
...
CREATE TABLE GOLD.Agg_PatientDemographics (
    ...
    num_patients INT,
    avg_visits INT,
    PRIMARY KEY (month_skey, gender, age_group, region),
    FOREIGN KEY (month_skey) REFERENCES GOLD.Dim_Date(date_skey)
);
